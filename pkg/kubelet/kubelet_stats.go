...
// stats from CRI.
func newCRIStatsProvider(runtimeService internalapi.RuntimeService, imageService internalapi.ImageManagerService, cadvisor cadvisor.Interface, resourceAnalyzer stats.ResourceAnalyzer) containerStatsProvider {
...
func (kl *Kubelet) GetCgroupStats(cgroupName string) (*statsapi.ContainerStats, *statsapi.NetworkStats, error) {
	info, err := getCgroupInfo(kl.cadvisor, cgroupName)
	if err != nil {
...
func (kl *Kubelet) RootFsStats() (*statsapi.FsStats, error) {
	...
	// Get the root container stats's timestamp, which will be used as the
	// imageFs stats timestamp.
	rootStats, err := getCgroupStats(kl.cadvisor, "/")
	if err != nil {
		return nil, fmt.Errorf("failed to get root container stats: %v", err)
	}
