...
func (sp *summaryProviderImpl) Get() (*statsapi.Summary, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to get root container and/or network stats: %v", err)
	}
	...
	nodeStats := statsapi.NodeStats{
		...
		Fs:        rootFsInfo,
		Runtime:   &statsapi.RuntimeStats{ImageFs: imageFsInfo},
	}
	...
	for sys, name := range systemContainers {
		...
		if err != nil {
			glog.Errorf("Failed to get system container stats for %q: %v\n", name, err)
			continue
		}
