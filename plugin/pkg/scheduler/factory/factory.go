...
//   - The pod has only its ResourceVersion/Spec.NodeName/Annotations updated.
func (c *configFactory) skipPodUpdate(pod *v1.Pod) bool {
	...
	// Compares the assumed pod with the pod update. If they are equivalent
	// (with certain fields excluded), this pod update will be skipped.
	f := func(pod *v1.Pod) *v1.Pod {
		...
		// not.
		p.Spec.NodeName = ""
		// Annotations must be excluded for the reason described in
