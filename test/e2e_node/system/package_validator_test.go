...
func TestValidatePackageVersion(t *testing.T) {
	...
	for _, test := range []struct {
		...
		},
		{
			...
			spec: SysSpec{
				PackageSpecs: []PackageSpec{
					{Name: "baz", VersionRange: ">=1.0"},
				},
			},
			err: errors.New("package \"baz\" does not exist"),
		},
	} {
		_, err := v.validate(test.spec, manager)
		if test.err == nil && err != nil {
			t.Fatalf("%s: v.validate(): err = %s", test.desc, err)
		}
		if test.err != nil {
			if err == nil {
				t.Fatalf("%s: v.validate() is expected to fail.", test.desc)
			} else if test.err.Error() != err.Error() {
				t.Fatalf("%s: v.validate(): err = %q, want = %q", test.desc, err, test.err)
			}
		}
	}
}
